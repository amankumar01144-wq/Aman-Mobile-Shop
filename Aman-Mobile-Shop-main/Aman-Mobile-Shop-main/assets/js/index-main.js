    ...
    productGrid.innerHTML = Object.entries(grouped).map(([category, catProducts]) => `
        ...
        return `
                        <div class="min-w-[150px] max-w-[150px] bg-white rounded-[2rem] p-2.5 product-card-shadow border border-gray-100 flex flex-col justify-between h-[280px] snap-start relative active:scale-95 transition-all" 
                            ...
                            <!-- Wishlist Icon -->
                            <button class="absolute top-3 right-3 z-10 w-6 h-6 bg-white/90 rounded-full flex items-center justify-center shadow-sm backdrop-blur-md active:scale-90 transition-all" onclick="event.stopPropagation(); window.toggleWishHome('${product.id}', this)">
                                <i class="${isInWishlist(product.id) ? 'fas text-red-500' : 'far text-gray-400'} text-[9px] fa-heart"></i>
                            </button>
